...
from SSR.training.training_utils import batchify_rays, calculate_segmentation_metrics, calculate_depth_metrics, compute_intrinsic_loss
from SSR.training.cluster import Cluster_Manager
...
class SSRTrainer(object):
    def __init__(self, config):
        ...
        self.w_c = 0.01
        self.b_f = 0.25
        self.no_cluster = config["train"]["no_cluster"]
        self.no_semantic_tree = config["train"]["no_semantic_tree"]
        self.no_intrinsic_loss = config["train"]["no_intrinsic_loss"]
    ...
    def volumetric_rendering(self, ray_batch):
        ...
        rgb_coarse, disp_coarse, acc_coarse, weights_coarse, depth_coarse, sem_logits_coarse, feat_map_coarse, albedo_coarse, shading_coarse, residual_coarse = \
        ...
        if self.N_importance > 0:
            ...
            rgb_fine, disp_fine, acc_fine, weights_fine, depth_fine, sem_logits_fine, feat_map_fine, albedo_fine, shading_fine, residual_fine = \
        ...
        ret['albedo_coarse'] = albedo_coarse
        ret['shading_coarse'] = shading_coarse
        ret['residual_coarse'] = residual_coarse
        if self.enable_semantic:
        ...
        if self.N_importance > 0:
            ...
            ret['albedo_fine'] = albedo_fine
            ret['shading_fine'] = shading_fine
            ret['residual_fine'] = residual_fine
            if self.enable_semantic:
    ...
    def step(
        ...
        albedo_coarse = output_dict["albedo_coarse"]
        shading_coarse = output_dict["shading_coarse"]
        residual_coarse = output_dict["residual_coarse"]
        #print(albedo_coarse.shape, shading_coarse.shape, residual_coarse.shape)
        ...
        if self.N_importance > 0:
            ...
            albedo_fine = output_dict["albedo_fine"]
            shading_fine = output_dict["shading_fine"]
            residual_fine = output_dict["residual_fine"]
            z_std = output_dict["z_std"] # N_rays
            if self.enable_semantic:
                sem_logits_fine = output_dict["sem_logits_fine"]
                sem_label_fine = logits_2_label(sem_logits_fine)

        #print(sampled_gt_semantic.shape, sampled_gt_semantic)

        #! 最开始的10000次迭代因为网络还没有输出albedo，这个时候是没有聚类约束的
        if (self.cluster_manager is not None) and (not self.no_cluster):
            with torch.no_grad():
                albedo = albedo_coarse
                sem_label = sem_label_coarse
                if self.N_importance > 0:
                    albedo = albedo_fine
                    sem_label = sem_label_fine #! 聚类约束就是计算网络输出的albedo和该albedo经过聚类转换之后的颜色之差
                target_cluster_albedo = self.cluster_manager.dest_color(albedo,sem_label)
        self.optimizer.zero_grad()

        img_loss_coarse = img2mse(rgb_coarse, sampled_gt_rgb)
        chroma_loss_coarse, residual_loss_coarse, reflect_sparsity_loss_coarse, shading_smooth_loss_coarse, far_reflect_loss_coarse, intensity_loss_coarse = compute_intrinsic_loss(albedo_coarse, shading_coarse, residual_coarse, sampled_gt_rgb, disp_coarse, acc_coarse,sampled_gt_semantic)
        ...
        if self.N_importance > 0:
            ...
            chroma_loss_fine, residual_loss_fine, reflect_sparsity_loss_fine, shading_smooth_loss_fine, far_reflect_loss_fine, intensity_loss_fine = compute_intrinsic_loss(albedo_fine, shading_fine, residual_fine, sampled_gt_rgb, disp_fine, acc_fine,sampled_gt_semantic)
            if self.enable_semantic and sematic_available:
        ...
        #! 这里同样是按照nerf那样分为fine和coarse网络的，这些约束同时加到两个网络的输出结果上，然后将两组loss整合起来
        total_img_loss = img_loss_coarse + img_loss_fine
        total_sem_loss = semantic_loss_coarse + semantic_loss_fine
        total_chroma_loss = chroma_loss_coarse + chroma_loss_fine
        total_residual_loss = residual_loss_coarse + residual_loss_fine
        total_reflect_sparsity_loss = reflect_sparsity_loss_coarse + reflect_sparsity_loss_fine
        total_shading_smooth_loss = shading_smooth_loss_coarse + shading_smooth_loss_fine
        total_far_reflect_loss = far_reflect_loss_coarse + far_reflect_loss_fine
        total_intensity_loss = intensity_loss_coarse + intensity_loss_fine
        #! 相关的一系列约束权重
        wgt_sem_loss = float(self.config["train"]["wgt_sem"])
        chroma_loss_weight = 1
        reflect_sparsity_loss_weight = float(self.config["train"]["w_n"])#0.01   #b0.04 #c0.01
        far_reflect_loss_weight = float(self.config["train"]["w_f"])#0.01
        shading_smooth_loss_weight = 1
        reflect_cluster_loss_weight = self.w_c#args.w_c#1
        if self.no_cluster:
            reflect_cluster_loss_weight = 0
        if(global_step<=100000): #! 残差约束的权重调整
            residual_loss_weight = 1
        else:
            residual_loss_weight = 0.02
        
        if(global_step<=50000): #! intensity约束的权重调整
            intensity_loss_weight = float(self.config["train"]["w_i1"])
        else:
            intensity_loss_weight = float(self.config["train"]["w_i2"])
            
        if self.no_intrinsic_loss:
            total_loss = total_img_loss + total_sem_loss*wgt_sem_loss
        else:
            total_loss = total_img_loss + total_sem_loss*wgt_sem_loss + total_chroma_loss*chroma_loss_weight + \
                total_residual_loss*residual_loss_weight + total_reflect_sparsity_loss*reflect_sparsity_loss_weight + \
                    total_shading_smooth_loss*shading_smooth_loss_weight + total_far_reflect_loss*far_reflect_loss_weight + \
                        total_intensity_loss*intensity_loss_weight

        total_reflect_cluster_loss = 0
        if (self.cluster_manager is not None) and (not self.no_cluster):
            reflect_cluster_loss_coarse = img2mse(albedo_coarse, target_cluster_albedo)
            reflect_cluster_loss_fine = img2mse(albedo_fine, target_cluster_albedo)
            total_reflect_cluster_loss = reflect_cluster_loss_coarse + reflect_cluster_loss_fine
            total_loss = total_loss + total_reflect_cluster_loss*reflect_cluster_loss_weight

        total_loss.backward()
        self.optimizer.step()
        
        if global_step%1000==0:
            print("img_loss:",total_img_loss, " weight:", 1)
            #print("albedo loss:", albedo_loss)
            print("chroma_loss:", total_chroma_loss, " weight:", chroma_loss_weight)
            print("reflect_sparsity_loss", total_reflect_sparsity_loss, " weight:",reflect_sparsity_loss_weight)
            print("far_reflect_loss:",total_far_reflect_loss, " weight:", far_reflect_loss_weight)
            print("shading_smooth_loss:",total_shading_smooth_loss, " weight:", shading_smooth_loss_weight)
            print("residual_loss:", total_residual_loss, " weight:", residual_loss_weight)
            print("reflect_cluster_loss:", total_reflect_cluster_loss, " weight:", reflect_cluster_loss_weight)
            print("intensity_loss:",total_intensity_loss, " weight:",intensity_loss_weight)
        ...
        if global_step % self.config["logging"]["step_vis_train"]==0 and global_step > 0:
            #! 每隔10000次对train数据进行渲染时，同时也更新一些训练参数
            n = float(global_step-self.config["logging"]["step_vis_train"])/float(self.config["train"]["N_iters"]-self.config["logging"]["step_vis_train"]*2)
            self.w_c = min(pow(0.1, 2.0-2.0*n), 1.0) #! 聚类约束权重，从最开始的0.01，逐渐上升到1
            self.b_f = min(pow(0.5, 2.0-2.0*n), 1.0) #! 聚类时的bandwidth系数，从0.25逐渐上升到1，这个值越小聚类得到的类别越多，当这个系数为1时，就是使用计算出的bandwidth。
            print('w_c:',self.w_c,' b_f',self.b_f)
            ...
            with torch.no_grad():
                ...
                rgbs, disps, deps, vis_deps, sems, vis_sems, sem_uncers, vis_sem_uncers, albedos, shadings, residuals, cluster_manager = self.render_path(self.rays_vis, save_dir=trainsavedir, update_cluster = not self.no_cluster,b_f = self.b_f)
                #  numpy array of shape [B,H,W,C] or [B,H,W]
            if not self.no_cluster:
                self.cluster_manager = cluster_manager
                cluster_manager_dir = os.path.join(trainsavedir, 'cluster')
                self.cluster_manager.save(cluster_manager_dir)
        ...
        if global_step % self.config["logging"]["step_val"]==0 and global_step > 0:
            ...
            with torch.no_grad():
                rgbs, disps, deps, vis_deps, sems, vis_sems, sem_uncers, vis_sem_uncers, albedos, shadings, residuals, _ = self.render_path(self.rays_test, save_dir=testsavedir)
    ...
    #! 渲染图像的同时，可以通过设定update_cluster来决定是否利用刚刚渲染好的图像对聚类进行更新
    def render_path(self, rays, save_dir=None, update_cluster = False, b_f = 0.5):
        ...
        for i, c2w in enumerate(tqdm(rays)):
            ...
            albedo = albedo.cpu().numpy().reshape((self.H_scaled, self.W_scaled, 3))
            shading = shading.cpu().numpy().reshape((self.H_scaled, self.W_scaled))
            residual = residual.cpu().numpy().reshape((self.H_scaled, self.W_scaled, 3))
            ...
            if update_cluster:
                sample_pixel = albedo[::2,::2,:]
                sample_pixels.append(sample_pixel.reshape(-1,3))
                sample_label = sem_label[::2,::2]
            ...
            if save_dir is not None:
                ...
                rgb8 = to8b_np(rgbs[-1])
                albedo8 = to8b_np(albedos[-1])
                shading8 = to8b_np(shadings[-1])
                residual8 = to8b_np(residuals[-1])
                ...
                albedo_filename = os.path.join(save_dir, 'albedo_{:03d}.png'.format(i))
                shading_filename = os.path.join(save_dir, 'shading_{:03d}.png'.format(i))
                residual_filename = os.path.join(save_dir, 'residual_{:03d}.png'.format(i))
                ...
                imageio.imwrite(albedo_filename, albedo8)
                imageio.imwrite(shading_filename, shading8)
                imageio.imwrite(residual_filename, residual8)
        ...
        shadings = np.stack(shadings, 0)
        residuals = np.stack(residuals, 0)
        ...
        if update_cluster: #! 在这里构造一组cluster
            sample_pixels = np.stack(sample_pixels, 0)
            sample_labels = np.stack(sample_labels, 0)
            if self.no_semantic_tree:
                cluster_manager = Cluster_Manager(class_num = 1)
            else:
                cluster_manager = Cluster_Manager(class_num = self.num_valid_semantic_class)
            ...
            for i, albedo in enumerate(tqdm(albedos)): # 同时输出当前渲染图像的albedo经过聚类之后的结果
                pixel = torch.from_numpy(albedo).reshape(-1,3).cuda()
                label = torch.from_numpy(sems[i]).reshape(-1,1).cuda()
                result = cluster_manager.dest_color(pixel,label)
                result = result.reshape(ori_shape).cpu().numpy()
                cluster_albedo = to8b_np(result)
                filename = os.path.join(save_dir, 'c{:03d}.png'.format(i))
                imageio.imwrite(filename, cluster_albedo)

                ori_shape = result.shape # 以及用聚类之后的albedo重新构造图像的结果
                edit_img = result.reshape(-1,3)*shadings[i].reshape(-1,1) +residuals[i].reshape(-1,3)
                edit_img = edit_img.reshape(ori_shape)
                edit_img = to8b_np(edit_img)
                filename = os.path.join(save_dir, 'edit{:03d}.png'.format(i))
                imageio.imwrite(filename, edit_img)

        return rgbs, disps, deps, vis_deps, sems, vis_sems, entropys, vis_entropys, albedos, shadings, residuals, cluster_manager