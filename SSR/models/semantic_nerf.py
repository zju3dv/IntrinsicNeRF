...
class Semantic_NeRF(nn.Module):
    ...
    def __init__(self, enable_semantic, num_semantic_classes, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, skips=[4], use_viewdirs=False,
        ...
        if use_viewdirs:
            ...
            #residual
            self.residual_linear = nn.Linear(W // 2, 3)
            #albedo
            self.albedo_linear1 = nn.Linear(W, W // 2)
            self.albedo_linear2 = nn.Linear(W // 2, 3)
            #shading
            self.shading_linear1 = nn.Linear(W, W // 2)
            self.shading_linear2 = nn.Linear(W // 2, 1)
    ...
    def forward(self, x, show_endpoint=False):
        ...
        if self.use_viewdirs:
            ...
            #shading
            shading = self.shading_linear1(h)
            shading = F.relu(shading)
            shading = self.shading_linear2(shading)
            ...
            residual = self.residual_linear(h)
            residual = F.sigmoid(residual)
            rgb  = albedo*shading + residual

            if self.enable_semantic:
                outputs = torch.cat([rgb, alpha, albedo, shading, residual, sem_logits], -1)
            else:
