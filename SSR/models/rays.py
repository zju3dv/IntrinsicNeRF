...
def sampling_index(n_rays, batch_size, h, w):
    ...
    index_h = index_hw//w
    index_w = index_hw%w
    #print(h,w,index_hw[0,0],index_hw.shape,index_h[0,0],index_w[0,0])
    nei = [-1, 0, 1]
    bias_w = torch.from_numpy(np.random.choice(nei, index_hw.shape[1])).to(index_hw.device)
    bias_h = torch.from_numpy(np.random.choice(nei, index_hw.shape[1])).to(index_hw.device)
    index_h += bias_h
    index_w += bias_w
    index_h = torch.clamp(index_h, 0, h-1)
    index_w = torch.clamp(index_w, 0, w-1)
    neighbor_hw = index_h*w + index_w
    index_hw = torch.cat((index_hw, neighbor_hw), 1)
    #print("2",neighbor_hw[0,0],neighbor_hw.shape,index_h[0,0],index_w[0,0])
    #print(index_hw.shape)
...
def create_rays(num_rays, Ts_c2w, height, width, fx, fy, cx, cy, near, far, c2w_staticcam=None, depth_type="z",
    ...
    print("rays_cam:",rays_cam.shape, rays_cam[0,1,11])
